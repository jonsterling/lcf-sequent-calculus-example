...
\paragraph{Backtracking}
  The \verb|orelse_| tactical enables a form of proof search
  procedure, but it cannot be used to implement backtracking. There
  are at least two ways to extend \LCF{} with support for
  backtracking; one way would be using continuations, but the most
  common way is to replace the type of tactics with something like the
  following:
\begin{minted}[mathescape]{sml}
...
\end{minted}
  ...
  will simply merge the results fof applying \emph{both} tactics into
  a single stream:
\begin{minted}[mathescape]{sml}
  fun par (t1, t2) goal =
...
\LCF{}-style proof assistants, because of the difficulty (and in some
cases, impossibility) of finding most-general unifiers. In
